...
List.prototype.fetch = function(req, res, context) {
  ...
  // Exact match relevance ordering: prepend a CASE rank so exact/prefix matches surface first
  if (req.query.exact) {
    var exactTerm = req.query.exact.toLowerCase().replace(/'/g, "''");
    ...
    if (rankAttr) {
      var relevance = Sequelize.literal(
        'CASE WHEN LOWER("' + rankAttr + '") = \'' + exactTerm + '\' THEN 0 ' +
        'WHEN LOWER("' + rankAttr + '") LIKE \'' + exactTerm + '%\' THEN 1 ' +
        'ELSE 2 END'
      );
