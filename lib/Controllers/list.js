...
List.prototype.fetch = function(req, res, context) {
  ...
  searchParams.forEach(function(searchData) {
    ...
    if (req.query[searchParam]) { //exists and is not ''
      var search = [];
      ...
      var searchAttributes =
        ...
        var searchString = req.query[searchParam];
        if (typeof searchString === 'string') searchString = [searchString];

        let newSearchStrings = [];
        for (let str of searchString) {
          if (str.indexOf('%') === -1 && str.indexOf(',AND,') === -1) {
            // Plain phrase: split into words and wrap with % wildcards
            newSearchStrings.push(...str.trim().split(/\s+/).filter(function(w) { return w; }).map(function(w) { return '%' + w + '%'; }));
          } else {
            newSearchStrings.push(...str.split(',AND,'));
          }
        }
        searchString = newSearchStrings;
        ...
        for (let str of searchString) {
          ...
          searchAttributes.forEach(function(attr) {
            ...
            let queries = [];
            for (let subStr of str.split(',OR,')) {
              if(searchOverride === "STARTS_WITH") {
                subStr = req.query[searchParam] + '%';
                searchOperator = Sequelize.Op.like;
              }

              let query = {}
              query[searchOperator] = subStr;
              queries.push(query)
            }

            let item = {}
            if (queries.length > 1) {
              item[attr] = {[Sequelize.Op.or]: queries};
            } else {
              item[attr] = queries[0];
            }
            searches.push(item);
          });
          queries.push({[Sequelize.Op.or]: searches});
        }
        criteria = Sequelize.and(criteria, {[Sequelize.Op.and]: queries});
    }
  });
  ...
  // Relevance ordering: surface exact/prefix/phrase matches before fallback sort.
  // Derive relevance phrase: use explicit `exact` param, or the plain `q` value,
  // or fall back to stripping % wildcards from the old %word%,AND,%word% format.
  var relevanceTerm = null;
  if (req.query.exact) {
    relevanceTerm = req.query.exact.toLowerCase().replace(/'/g, "''").trim();
  } else if (req.query.q) {
    var qVal = req.query.q;
    if (qVal.indexOf('%') === -1 && qVal.indexOf(',AND,') === -1) {
      // New plain format — use directly
      relevanceTerm = qVal.toLowerCase().replace(/'/g, "''").trim();
    } else {
      // Old %word%,AND,%word% format — strip wildcards and rejoin
      var derivedWords = qVal.split(',AND,')
        .map(function(p) { return p.replace(/%/g, '').toLowerCase().trim(); })
        .filter(function(w) { return w.length > 0; });
      if (derivedWords.length > 0)
        relevanceTerm = derivedWords.join(' ').replace(/'/g, "''");
    }
  }
  if (relevanceTerm) {
    var stringAttrs = getKeys(model.rawAttributes).filter(function(k) {
      var type = model.rawAttributes[k].type;
      return (type instanceof Sequelize.STRING || type instanceof Sequelize.TEXT);
    });
    var rankAttr = stringAttrs.indexOf('name') !== -1 ? 'name' : stringAttrs[0];
    if (rankAttr) {
      var relevance = Sequelize.literal(
        'CASE WHEN LOWER("' + rankAttr + '") = \'' + relevanceTerm + '\' THEN 0 ' +
        'WHEN LOWER("' + rankAttr + '") LIKE \'' + relevanceTerm + '%\' THEN 1 ' +
        'WHEN LOWER("' + rankAttr + '") LIKE \'%' + relevanceTerm + '%\' THEN 2 ' +
        'ELSE 3 END'
      );
      options.order = [[relevance, 'ASC']].concat(options.order || []);
    }
  }
